...
public class GuestbookController {
	...
	public String list(@RequestParam(name = "start", required = false, defaultValue = "0") int start, ModelMap model,
		...
		/*
		 * String value = null; boolean find = false; Cookie[] cookies =
		 * request.getCookies(); if(cookies != null) { for(Cookie cookie : cookies) {
		 * if("count".equals(cookie.getName())) { find = true; value =
		 * cookie.getValue(); break; } } }
		 * 
		 * if(!find) { value = "1"; } else { try { int i = Integer.parseInt(value);
		 * value = Integer.toString(++i); } catch(Exception ex) { value = "1"; } }
		 */
		...
		List<Guestbook> list = guestbookService.getGuestbooks(start); // start로 시작하는 방명록 구하기
